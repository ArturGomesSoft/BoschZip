...
{
    [TestFixture]
    public class ZipSettingsTests_SMTP
    {
        ...
        public void Setup()
        {
            settings = ZipSettings.FromArgs(ResourceClasses.GetArgs_SMTP());
        }
        ...
        [Test]
        public void ZipSettings_OutputType_SMTP()
        {
            Assert.IsTrue(settings.OutputType == OutputTypes.SMTP, "Output type should be 'SMTP'.");
        }
        ...
        [Test]
        public void ZipSettings_OutputOption_Email()
        {
            string email = "my_email@my.server.com";
            Assert.IsTrue(settings.OutputOption.Equals(email, StringComparison.OrdinalIgnoreCase), "Incorrect email address loaded.");
        }
